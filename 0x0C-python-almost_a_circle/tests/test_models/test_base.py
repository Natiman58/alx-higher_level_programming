...
class LearnTest(unittest.TestCase):
    ...
    def setUp(self):
        """
            A setup method that is called evey time before\
            a test function is called
        """
        Base.__Base__nb_objects = 0

    def test_1(self):
        """Check for each value of id"""
        b1 = Base()
        ...
        b6 = Base(-23)
        self.assertEqual(b6.id, -23)
        b7 = Base(134567890)
        self.assertEqual(b7.id, 134567890)

    def test_1_1(self):
        """Check the type and instance of the class"""
        b8 = Base()
        self.assertEqual(type(b8), Base)
        self.assertTrue(isinstance(b8, Base))

    def test_15_0(self):
        """Testing the to_jason_string method a sample dict 'd'"""
        d = {'x': 3, 'width': 14, 'id': 17, 'height': 4, 'y': 9}
        json_str = Base.to_json_string([d])
        self.assertTrue(isinstance(d, dict))
        self.assertTrue(isinstance(json_str, str))
        self.assertCountEqual(
            json_str, '[{"x": 3, "width": 14, "id": 17, "height": 4, "y": 9}]')

        json_str_1 = Base.to_json_string([])
        self.assertEqual(json_str_1, "[]")

        json_str_2 = Base.to_json_string(None)
    ...
    def test_15_2(self):
        ...
        str_2 = "to_json_string() takes 1 positional argument but 2 were given"
        with self.assertRaises(TypeError) as x:
            Base.to_json_string([{1, 2}], [{3, 4}])
        self.assertEqual(str_2, str(x.exception))
